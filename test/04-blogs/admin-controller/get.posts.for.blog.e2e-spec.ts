...
describe('BlogsAdminController - getPostsForBlog() (GET: /sa/blogs/{blogId}/posts)', () => {
  ...
  it('should return all posts from a specific blog.', async () => {
    ...
    const resGetPosts: Response = await request(server)
      .get(`/${GLOBAL_PREFIX}/sa/blogs/${blog.id}/posts`)
    ...
    if (testLoggingEnabled) {
      TestLoggers.logE2E(
        ...
        'Test №1: BlogsAdminController - getPostsForBlog() (GET: /sa/blogs/{blogId}/posts)',
      );
    }
  });
  ...
  it('should return all entries from a specific blog using the pagination values provided by the client.', async () => {
    ...
    if (testLoggingEnabled) {
      TestLoggers.logE2E(
        resGetPosts.body,
        resGetPosts.statusCode,
        'Test №2: BlogsAdminController - getPostsForBlog() (GET: /sa/blogs/{blogId}/posts)',
      );
    }
  });
  ...
  it('should return a 404 error if the blog does not exist.', async () => {
    ...
    const resGetPosts_1: Response = await request(server)
      .get(`/${GLOBAL_PREFIX}/sa/blogs/${incorrectBlogId}/posts`)
      .set('Authorization', adminCredentialsInBase64) // 🔸 админ авторизован
    ...
    if (testLoggingEnabled) {
      TestLoggers.logE2E(
        ...
        'Test №3: BlogsAdminController - getPostsForBlog() (GET: /sa/blogs/{blogId}/posts)',
      );
    }
  });

  it('should return a 401 error if the admin is not logged in.', async () => {
    // 🔻 Создаем один тестовый блог
    const [blog]: BlogViewDto[] = await blogsTestManager.createBlog(1);
    ...
    // 🔻 Повторяем запрос с корректной авторизацией администратора
    const resGetPosts_2: Response = await request(server)
      .get(`/${GLOBAL_PREFIX}/sa/blogs/${blog.id}/posts`)
      .set('Authorization', adminCredentialsInBase64) // 🔸 корректный base64 логин:пароль
      .expect(HttpStatus.OK); // 🔸 ожидаем 200 OK

    // 🔸 Убеждаемся, что был возвращен ровно 1 пост
    ...
    if (testLoggingEnabled) {
      TestLoggers.logE2E(
        resGetPosts_1.body,
        resGetPosts_1.statusCode,
        'Test №4: BlogsAdminController - getPostsForBlog() (GET: /sa/blogs/{blogId}/posts)',
      );
    }
  });
  ...
  it('should return error 400 if the BlogId is not valid.', async () => {
    ...
    const resGetPosts_1: Response = await request(server)
      .get(`/${GLOBAL_PREFIX}/sa/blogs/${invalidBlogId}/posts`)
      .set('Authorization', adminCredentialsInBase64) // 🔸 Авторизация присутствует
    ...
    if (testLoggingEnabled) {
      TestLoggers.logE2E(
        resGetPosts_1.body,
        resGetPosts_1.statusCode,
        'Test №5: BlogsAdminController - getPostsForBlog() (GET: /sa/blogs/{blogId}/posts)',
      );
    }
  });
});
